...
public class QualificationController {
  ...
  private void initialize() {
    ...
    choiceBoxVerdict
        ...
        .addListener(
            (obs, oldVal, newVal) -> {
              if (newVal != null) nuit.setVerdictGlobal(newVal);
            });
    labelVerdictGlobal
        .textProperty()
        .bind(
  ...
  private void ecouter() {
    ...
    Sequence sel = tableView.getSelectionModel().getSelectedItem();
    if (sel != null) sel.setStatut("Écoutée");
  }
  ...
  private void enregistrerVerdict() {
    ...
    // sélectionné.
    if (nuit.verdictGlobalProperty().isEmpty().not().get())
